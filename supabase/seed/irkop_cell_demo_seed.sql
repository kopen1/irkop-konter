-- DEMO DATA ONLY.
-- Run after at least one authenticated Supabase user exists.
...
begin
  ...
  if owner_id is null then
    raise exception 'Demo seed requires at least one auth.users account.';
  end if;

  if exists (select 1 from public.irkop_cell_businesses where is_demo = true) then
    raise notice 'Demo business already exists. Seed skipped.';
    return;
  end if;
  ...
  for i in 1..30 loop
    insert into public.irkop_cell_products(business_id,name,category,sku,sell_price,stock)
    values (b,'Produk Demo '||i,case when i%3=0 then 'Aksesoris' when i%3=1 then 'Pulsa' else 'Service' end,'DEMO-SKU-'||lpad(i::text,4,'0'),(25000+random()*475000)::numeric(14,2),(5+random()*95)::numeric(14,2));
  end loop;
  ...
  for i in 1..150 loop
    ...
    insert into public.irkop_cell_transactions(business_id,outlet_id,customer_id,transaction_no,payment_method,status,total,transaction_at)
    values (b,o,c,'DEMO-'||lpad(i::text,5,'0'),methods[1+floor(random()*4)::int],'completed',(50000+random()*950000)::numeric(14,2),now()-((150-i)||' hours')::interval)
    returning id into t;
    ...
    select b,o,t,'in',total,'Demo transaction' from public.irkop_cell_transactions where id=t;
  end loop;
end $$;
