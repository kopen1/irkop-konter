-- IRKOP Konter demo seed.
-- Run after at least one authenticated Supabase user exists.
-- The seed uses the first business owned by that user, so the Flutter app
-- immediately sees the demo records in the currently selected tenant.
...
begin
  ...
  select id into o
  from public.irkop_cell_outlets
  where business_id=b
  order by created_at asc
  limit 1;
  if o is null then
  ...
  -- Categories used by the product catalog.
  ...
  -- Create products only when this tenant has no products, keeping the seed repeatable.
  if not exists (select 1 from public.irkop_cell_products where business_id=b) then
    for i in 1..30 loop
      ...
      values (
        b,
        'Produk Demo '||i,
        case when i%5=0 then 'Voucher' when i%3=0 then 'Aksesoris' when i%3=1 then 'Pulsa' else 'Service' end,
        (select id from public.irkop_cell_product_categories where business_id=b and name=
          case when i%5=0 then 'Voucher' when i%3=0 then 'Aksesoris' when i%3=1 then 'Pulsa' else 'Service' end),
  ...
  -- Add a small transaction history only when this tenant has no transactions.
