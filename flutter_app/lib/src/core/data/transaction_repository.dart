...
class DashboardMetrics {
  const DashboardMetrics({required this.todayRevenue,required this.todayTransactions,required this.activeCredit,required this.cashBalance,required this.recentTransactions});
  final double todayRevenue,activeCredit,cashBalance; final int todayTransactions; final List<TransactionSummary> recentTransactions;
}
class TransactionRepository {
  ...
  Future<TransactionSummary> createManualTransaction({required String businessId,required String outletId,required double total,required String paymentMethod,required DateTime transactionAt,String? notes}) async {
    ...
    final result=await _client.rpc('irkop_cell_manual_transaction',params:{'p_business_id':businessId,'p_outlet_id':outletId,'p_total':total,'p_payment_method':paymentMethod,'p_transaction_at':transactionAt.toUtc().toIso8601String(),'p_notes':notes});
    final data=result is List?result.first:result;return TransactionSummary.fromMap(Map<String,dynamic>.from(data as Map));
  }
  Future<DashboardMetrics> loadDashboard(String businessId) async {final transactions=await loadTransactions(businessId);final now=DateTime.now(),today=DateTime(now.year,now.month,now.day);final completed=transactions.where((t){final d=t.transactionAt.toLocal();return !d.isBefore(today)&&t.status=='completed';}).toList();double activeCredit=0,cashBalance=0;try{final credits=await _client.from('irkop_cell_credit_balances').select('balance').eq('business_id',businessId).gt('balance',0);activeCredit=credits.fold<double>(0,(sum,row)=>sum+((row['balance'] as num).toDouble()));}catch(_){}try{final cash=await _client.from('irkop_cell_cash_mutations').select('mutation_type,amount').eq('business_id',businessId);cashBalance=cash.fold<double>(0,(sum,row){final amount=(row['amount'] as num).toDouble();return sum+((row['mutation_type']=='in')?amount:-amount);});}catch(_){}return DashboardMetrics(todayRevenue:completed.fold<double>(0,(s,t)=>s+t.total),todayTransactions:completed.length,activeCredit:activeCredit,cashBalance:cashBalance,recentTransactions:transactions.take(8).toList());}
