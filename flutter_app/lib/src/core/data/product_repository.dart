...
class ProductRepository {
  ...
  Future<void> createProduct({required String businessId,required String name,required String category,required double price,required double stock})=>_client.from('irkop_cell_products').insert({'business_id':businessId,'name':name.trim(),'category':category.trim().isEmpty?'Lainnya':category.trim(),'sell_price':price,'stock':stock,'is_active':true});
  Future<void> updateProduct({required String id,required String businessId,required String name,required String category,required double price,required double stock})=>_client.from('irkop_cell_products').update({'name':name.trim(),'category':category.trim().isEmpty?'Lainnya':category.trim(),'sell_price':price,'stock':stock}).eq('id',id).eq('business_id',businessId);
